...
# Install / show / remove ONLY the 10:00 IST cache-monitor cron line.
# (Filename still says "noon": the job ran at 12:00 until 2026-09-07. The name is
#  kept because ensure_scheduler.sh calls it by path; the TIME below is the truth.)
#
# Deliberately narrow: it touches exactly one crontab line, tagged
# "# ambimat-cache-monitor". Every other crontab line — including the 08:00
# site-monitor, the 09:00 SEO job and the */30 watchdog — is passed through
# untouched. Removing this line disables the cache job and nothing else.
#
...
CRON_LINE="0 10 * * * $CM/run_cache_monitor_daily.sh >> $REPORTS/cron.log 2>&1 $MARKER"
...
case "$cmd" in
  install)
    ...
    echo "Installed cache-monitor cron entry:"
    echo "  $CRON_LINE"
  ...
  remove)
    ...
    echo "Removed the 10:00 cache-monitor cron entry. The 08:00, 09:00 and watchdog lines are untouched."
